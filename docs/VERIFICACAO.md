...
`python scripts/adaptacao/comparacao_erros.py` roda o programa adaptado com a **entrada impressa de cada tabela de 1973** e compara cada saída legível com a impressa. São 13 casos: todas as tabelas de saída do relatório (pp. 29 a 68). As pp. 44 e 47 do scan trazem a mesma impressão, então contam como um caso só.

Saídas, em `output/verificacao/` (geradas, fora do Git):
...
As transcrições estão em `data/tabelas_1973/` (uma por página, com o cabeçalho) e as leituras brutas, com os glifos ambíguos marcados, em `data/tabelas_1973/leituras/`.
...
- **circular**: algum `DATA` ou alguma entrada do cabeçalho foi decidido usando esta célula (`scripts/adaptacao/circularidade.py`). Mostra só que a decisão é coerente.
...
**Independentes: 89 % indistinguíveis do original, 95 % no critério, erro mediano de 0,26 unidade.** Sem o M1, cuja geometria do cabeçalho não fecha (abaixo): 91 % e 97 %.
...
| 44/47 | M1 | 159 | 60 | 50 % | 50 % | 1,99 | Magnus, Cmq e Clp fecham; CX, CX2, CNα, CPN e CMα não |
| 50 | 105 mm XM380E5 | 231 | 217 | 93 % | 98 % | 0,33 | quase nada circular: o teste mais limpo |
| 53 | 5"/38 NAVY | 226 | 180 | 95 % | 98 % | 0,33 | fechou com o cartão C205 (NOTAS, T15) |
...
| CNA | 9 | 86 | 77 % | 91 % | 0,28 | |
| CX2 | 11 | 95 | 72 % | 91 % | 0,35 | subtrai o CNα: herda o erro dele |
| CPN | 10 | 81 | 79 % | 94 % | 0,22 | |
| CMA | 10 | 87 | 52 % | 82 % | 0,49 | = (VCG − CPN)·CNα: amplia 2 a 3 vezes os erros do CNα e do CPN (abaixo) |
...
**O CMα é erro nosso ou do SPINNER?** Contra as tabelas de 1973, o erro é nosso e pequeno (mediana de 0,0005). A tabela impressa é coerente consigo mesma (CMα = (VCG − CPN)·CNα fecha a 0,002), e o erro do nosso CMα vem, meio a meio, do erro do CNα (multiplicado pelo braço VCG − CPN ≈ 2 cal) e do erro do CPN (multiplicado pelo CNα ≈ 2,6). Nas 65 linhas em que CNα e CPN estão ambos no critério, o CMα também fica dentro de ±0,004. O maior desvio (5"/38 supersônico) era o cartão C205, que faltava na adaptação (NOTAS, T15). Já o erro do próprio SPIN-73 contra a realidade é outra ordem de grandeza: a Tabela 1 do relatório (p. 28) dá erro provável de 0,12 a 0,17 no CMα contra o experimento.

## Duas perguntas diferentes

Este resultado responde **"a adaptação reproduz o SPIN-73 de 1973?"**. Nos blocos lidos diretamente do código e dos DATA, sim, no nível do arredondamento da impressão, em doze dos treze casos. O ponto mais fraco é o CMα, que acumula os erros do CNα e do centro de pressão.

A outra pergunta — **"o SPIN-73 acerta a realidade?"** — está em [voo_livre/BENCHMARKS.md](voo_livre/BENCHMARKS.md) e [voo_livre/RECALIBRACAO_MR1833.md](voo_livre/RECALIBRACAO_MR1833.md). Contra voo livre da família 7,62 NATO, o modelo de 1973 superestima o Magnus (+0,3 a +0,5 em p·d/2V) e o amortecimento Cmq (2,5 vezes em Mach 1,2, 10 % em Mach 2 a 2,5), erra o CNα do projétil mais curto (+0,21) e, de Mach 2,0 a 2,5, põe o centro de pressão 8 a 13 % atrás do medido. Contra o Ball M2 do Hitchcock (calibre 0.30, Mach 2,5), o Cmq erra 3 %; o CMα, 20 % (Mach 2,3, com o meplat suposto). Esses erros são do modelo original, não da adaptação; no centro de pressão e no CMα, com a ressalva de que parte do XC acima de Mach 1,1 (o XC15 de Mach 1,2 a 5,0, o XC12 de 1,35 e 1,5 e o XC1 de 2,5) foi decidida pelas tabelas de 1973, não lida.
