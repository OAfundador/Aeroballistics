...
As dependências só apontam para baixo: as adições importam do núcleo, e o núcleo não importa nenhuma adição. Sem opções, `spin73.Aerodinamica(p)` é a tabela canônica interpolada em Mach, e a linha de comando abre a saída com `Modo: canônico (SPIN-73 de 1973)`.
...
| `cli` | o comando `spin73` | todos | interface |
...
```
src/spin73/        o pacote (acima)
tests/             a suíte: python -m pytest (reconstrução, tabelas de 1973, biblioteca, adições)
scripts/
  caminhos.py      os caminhos do repositório; põe src/ e as pastas de scripts no sys.path
  reconstrucao/    como cada DATA foi decidido: análises por bloco, leitura e conferência das
...
- **Nada decidido valida a si mesmo.** `scripts/reconstrucao/circularidade.py` lê as decisões registradas em `spin73.dados.x?_lidos`, em `dados_cna.py` e nas linhas `decidir:` dos CSV das tabelas, e marca as células circulares. Elas ficam fora das estatísticas dos testes e da verificação.
...
- **Uma correção nova:** uma classe com `nome` e `aplicar(t, p, ctx)` ([BIBLIOTECA.md](BIBLIOTECA.md)), registrada com `spin73.correcoes.registrar` para ser chamada pelo nome. Se ela for ajustada a dados, o ajuste fica em `scripts/` e o resultado num arquivo que a biblioteca só lê, como `correcoes/voo_livre.json`.
...
- **Uma releitura de `DATA`:** no `src/spin73/dados/x?_lidos.py` do bloco, com a classe (verificado, decidido pelo modelo ou pendente) e a evidência. Se ela for decidida por alguma tabela, registre isso nas estruturas que `circularidade.py` lê (`DECIDIDOS`, `CORRECOES`, `RECUPERADOS`) e confira que as células usadas saem da validação.
- **Uma tabela de 1973 relida:** a leitura bruta em `data/tabelas_1973/leituras/`, o CSV gerado por `scripts/reconstrucao/resolver_glifos.py` e conferido, sem o modelo, por `scripts/reconstrucao/verificar_identidades.py`.
