...
# --------------------------------------------------------------------------- fontes
def _nato556(out):
    arq = DADOS / "nato556_mccoy1985.csv"
    duv = _duvidosas(arq)
    for r in _csv(arq):
        nome = r["PROJETIL"]
        geo = GEO[nome]
...
def _m33(out):
    ...
    for r in _csv(DADOS / "m33_mccoy1990.csv"):
...
def _m483(out):
    ...
    for r in _csv(DADOS / "m483a1_whyte1991.csv"):
        M = _f(r["MACH"])
...
def _nato762(out):
    ...
    geo62 = {r["projetil"]: r for r in _csv(DADOS / "mr1833_geometria.csv")}
    d_in = 0.308
...
def _t203(out):
    ...
    for r in _csv(DADOS / "t203_karpov1955.csv"):
        if r["PROJETIL"] != nome:
            continue
...
def _xm617(out):
    ...
    for r in _csv(DADOS / "xm617_brandon1969.csv"):
        M, at, cd = _f(r["MACH"]), _f(r["AT"]), _f(r["CD"])
        if at > GUINADA_MAX:
            continue
        sen2 = np.sin(np.radians(at)) ** 2
