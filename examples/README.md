...
`entradas/` holds input cards in the `KEY = value` format that `aeroballistics --entrada` also reads: [m437.txt](entradas/m437.txt), the report's own card, and [m855_metrico.txt](entradas/m855_metrico.txt), in metric units with the mass estimate turned on.
