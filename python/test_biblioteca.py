"""A interface de biblioteca (spin73.Aerodinamica e spin73.correcoes)."""
import numpy as np
import pytest
...
P556 = spin73.Projetil(VL=4.05, VN=1.90, VB=0.40, VCG=2.51, OR=7.9, DM=0.12, BD=1.00,
                       DIA=0.224, nome="M855")


def test_sem_correcao_e_o_programa_de_1973():
    a = spin73.Aerodinamica(spin73.M437)
    t = spin73.tabela(spin73.M437)
    for j, M in enumerate(spin73.MACH_GRID):
        c = a(M)
...
def test_interpolacao_linear_e_vetorizada():
    ...
    assert v.shape == (4,)
    assert np.allclose(v, np.interp(M, spin73.MACH_GRID, a.tabela["CNA"]))
    assert isinstance(a.coeficiente("CNA", 2.25), float)


def test_convencao_moderna():
    s73 = spin73.Aerodinamica(spin73.M437)(2.0)
    mod = spin73.Aerodinamica(spin73.M437, convencao="moderna")(2.0)
...
def test_fora_da_faixa():
    assert spin73.Aerodinamica(spin73.M437)(7.0).CX0 == pytest.approx(spin73.tabela(spin73.M437)["CX"][-1])
    assert np.isnan(spin73.Aerodinamica(spin73.M437, fora_da_faixa="nan")(7.0).CX0)
    with pytest.raises(ValueError):
        spin73.Aerodinamica(spin73.M437, fora_da_faixa="erro")(7.0)


def test_correcao_por_nome_igual_ao_experimental():
    import os
    import sys
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), "experimental", "correcao"))
    a = spin73.Aerodinamica(P556, "voo_livre")
...
def test_escolher_so_um_coeficiente():
    a = spin73.Aerodinamica(P556, "voo_livre:CX0")
...
def test_correcao_de_escala_exige_diametro():
    p = spin73.Projetil(VL=4.05, VN=1.90, VB=0.40, VCG=2.51)
    with pytest.raises(ValueError):
        spin73.Aerodinamica(p, "voo_livre")
    assert spin73.Aerodinamica(p, "voo_livre", d_mm=5.69).contexto.d_mm == 5.69
...
def test_correcao_do_usuario_e_registro():
    ...
    a = spin73.Aerodinamica(spin73.M437, CmaMais10())
    ...
    assert np.allclose(a.tabela["CPN"], spin73.M437.VCG - a.tabela["CMA"] / a.tabela["CNA"])
    ...
    b = spin73.Aerodinamica(spin73.M437, ["cma_mais_10"])
    assert np.allclose(b.tabela["CMA"], a.tabela["CMA"])
    with pytest.raises(KeyError):
        spin73.Aerodinamica(spin73.M437, "nao_existe")


def test_encadeamento_na_ordem_dada():
    a = spin73.Aerodinamica(P556, ["voo_livre:CX0", "voo_livre:CNA"])
    b = spin73.Aerodinamica(P556, "voo_livre")
    for col in ("CX", "CNA", "CPN"):
        assert np.allclose(a.tabela[col], b.tabela[col])


def test_dados_alternativos():
    k = spin73.CoefAjuste.do_listing()
    k.a[0] = k.a[0] + 0.01                      # XA1 + 0,01 -> CX + 0,01 em todo Mach
    a = spin73.Aerodinamica(spin73.M437, dados=k)
    assert np.allclose(a.tabela["CX"], a.tabela_original["CX"])      # original = com os mesmos dados
    assert np.allclose(a.tabela["CX"], spin73.tabela(spin73.M437)["CX"] + 0.01)
...
def test_api_antiga_continua():
    """O que o módulo spin73.py exportava continua acessível."""
    for nome in ("tabela", "formatar", "avisos", "Projetil", "CoefAjuste", "M437", "MACH_GRID",
        ...
        assert hasattr(spin73, nome), nome
