...
Na leitura (tabelas/leituras/pNN.txt), cada célula é escrita como foi vista, com classes:
...
das contagens de validação). Nenhum DATA e nenhum resultado do modelo entram aqui.
    ...
    python tabelas/resolver_glifos.py 41      -> tabelas/p41_*.csv (a partir da leitura)
"""
import itertools
import os
import re
...
def converter(pagina):
    (leitura,) = [f for f in os.listdir(os.path.join(AQUI, "leituras")) if f.startswith(f"p{pagina:02d}_")]
    linhas = open(os.path.join(AQUI, "leituras", leitura), encoding="utf-8").read().splitlines()
    ...
    saida = os.path.join(AQUI, leitura.replace(".txt", ".csv"))
