...
- Benchmarks de voo livre (archive.org, DTIC, todos com liberação pública), transcritos em `data/voo_livre/`:
  ...
  \* PDFs "MRC" da DTIC: o texto fica numa máscara JBIG2, que o Pillow não lê e o leitor de PDF do Windows também não. Foram lidos com `scripts/leitura/pdf_paginas.py`, que usa o decodificador JBIG2 em Python puro de `scripts/leitura/jbig2.py` (qualquer leitor de PDF baseado em PDFium ou pdf.js mostra as mesmas páginas).
