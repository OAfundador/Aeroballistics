...
Tolerância: ±0,0015 nas colunas de 3 casas (o arredondamento da impressão); ±1 no último dígito nas demais. `python -m pytest -q python` roda 256 testes.
...
**Benchmarks de voo livre** (`python/experimental/benchmarks/`): 155 mm M101 (Karpov 1964), 155 mm M483A1 (Whyte 1991), .50 M33 (McCoy 1990), 5,56 NATO (McCoy 1985), 7,62 match (McCoy 1988), 30 mm XM788/XM788E1/XM789 (McCoy 1980, 1982), 175 mm T203 em modelo de 90 mm (Karpov 1955) e 152 mm XM617, um cone-cilindro (Brandon 1969), com as convenções de cada fonte convertidas. No supersônico, o SPIN-73 acerta CMα, CNα e CX0 a poucos por cento nos projéteis de artilharia e no cone-cilindro, mas subestima o CMα das balas de boattail longo (.50: 21 %; 7,62 match: 9 a 14 %). No subsônico, o CX0 erra de −33 % a +25 % conforme a forma e o tamanho.

**Correção empírica** (`python/experimental/correcao/`): com os benchmarks e a 7,62 NATO (dez grupos de projéteis, 1391 valores medidos), o SPIN-73 é corrigido naquilo que não tem, a escala do projétil (número de Reynolds). Uma correção só entra se reduzir o erro em projéteis que o ajuste não viu (validação cruzada deixando um grupo de fora). O CX0 entra nos três regimes e é a peça firme (erro supersônico de 6,6 % para 4,8 %, subsônico de 19,3 % para 15,1 %, melhorando 7 de 9–10 grupos). O CNα subsônico e o Cmq transônico também passam, mas no limite da regra. O CMα e o Magnus não melhoram com nenhuma forma simples e ficam como no SPIN-73. A correção é opcional na biblioteca (`correcoes="voo_livre"` ou só `"voo_livre:CX0"`).
